...
# NOTE: This code is designed to work in both local and cloud environments
# by first checking Streamlit's secrets and then falling back to environment variables.
class BigQueryClient:
    ...
    def __init__(self):
        ...
        try:
            # 1. First, try to load credentials using Streamlit's built-in secrets management.
            # This is the standard way to load secrets from secrets.toml.
            service_account_info = st.secrets["gcp_bigquery_service_account"]
            st.success("BigQuery client initialized successfully from secrets.toml.")
        except KeyError:
            # 2. If secrets.toml is not found or the key is missing, fall back to environment variable.
            st.warning("Could not find 'gcp_bigquery_service_account' in secrets.toml. Attempting to load from environment variable.")
            try:
                service_account_info_json = os.environ.get('GCP_BIGQUERY_SERVICE_ACCOUNT')
                if not service_account_info_json:
                    raise ValueError("Environment variable GCP_BIGQUERY_SERVICE_ACCOUNT not found.")
                
                service_account_info = json.loads(service_account_info_json)
                st.success("BigQuery client initialized successfully from environment variable.")
            except Exception as e:
                st.error(f"Failed to initialize BigQuery client from environment variable: {e}")
                return
        except Exception as e:
        ...
        try:
            ...
            st.error(f"Failed to initialize BigQuery client with credentials: {e}")
            self.client = None
