def preprocessing_garmin_data(username, password, start_date, end_date, headers):
    ...
    for dt in date_range:
        try:
            ...
            logger.warning(f"No body battery data for {dt}: {e}")
        try:
            ...
            logger.warning(f"No RHR data for {dt}: {e}")
        try:
            ...
            logger.warning(f"No steps data for {dt}: {e}")
        try:
            ...
            logger.warning(f"No user summary for {dt}: {e}")
    ...
    # Merge datasets into one final dataframe
    ...
    # Align columns to match Google Sheets headers (provided from app.py)
    for col in headers:
